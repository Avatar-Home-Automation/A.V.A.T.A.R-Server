...
async function updateVersion(version) {
	...
	case 'win32':
		...
		fs.copySync(path.resolve(__dirname, 'lib', 'newVersion', 'win32', 'step-1.ps1'), path.resolve(__dirname, 'tmp', `${version}.ps1`));
		fs.writeFileSync(path.resolve(__dirname, 'tmp', 'shell.bat'), cmd, 'utf8');

		const opened = await Avatar.Interface.shell().openPath(path.resolve(__dirname, 'tmp', "shell.bat"));
		if (opened) {
	...
	case 'linux':
        ...
        fs.copySync(path.resolve(__dirname, 'lib', 'newVersion', 'linux', 'step-1.ps1'), path.resolve(__dirname, 'tmp', `${version}.ps1`));
		fs.writeFileSync(path.resolve(__dirname, 'tmp', 'shell.sh'), cmd, 'utf8');
        fs.chmodSync(path.resolve(__dirname, 'tmp', 'shell.sh'), "755");
        const batch = `gnome-terminal --working-directory=${path.resolve(__dirname, 'tmp')} -- ./shell.sh`;

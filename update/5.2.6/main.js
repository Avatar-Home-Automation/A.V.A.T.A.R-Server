...
function createWindow () {
  ...
  fs.readJson(path.resolve(__dirname, 'assets/config/nodes/Main.json'), async (err, mainStyle) => {
      ...
      ipcMain.handle('validateCronExpression', async (event, arg) => {return await validateCronExpression(arg) });
...
async function scenarioStudio() {
  ...
  let infos = {plugins: [], clients: []};
  infos.plugins = await Avatar.pluginLibrairy.getPlugins();
  infos.plugins = _.pluck(infos.plugins, 'name');
  infos.clients = await Avatar.Socket.getClients();
  infos.clients = _.reject(infos.clients, client => { return client.is_mobile === true});
  ...
  scenarioStudioWindow.once('ready-to-show', async () => {
    ...
    try {
      scenarioStudioWindow.webContents.send('init-scenario', infos);
    } catch (err) {

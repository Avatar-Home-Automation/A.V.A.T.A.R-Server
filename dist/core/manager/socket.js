...
class Client {
  constructor(name, obj, loop_mode, server_speak, ip, port, mobile, language) {
...
function setSockets (http) {
    ...
    // client connection
		obj.on('client_connect',  (name, ip, port, server_speak, loop_mode, mobile, language) => {
      ...
      let client = new Client (name, obj, loop_mode, server_speak, ip, port, mobile, language);
      ...
      if (intercomTo.length > 0 || client) {
        ...
        for (let i in pluginList) {
          if (pluginList[i]._script.subclassIntercom) {
              copyIntercomFile (obj, pluginList[i].name, () => {
...
function copyIntercomFile(element, plugin, callback) {
  const sharedFolder = path.resolve(Config.modules[plugin].speech.sharedFolder, 'intercom');
  fs.ensureDirSync(sharedFolder);
  let stream = ss.createStream();
	ss(element).emit('copyIntercomFile', stream, callback);
  let intercomFile = path.resolve (sharedFolder, 'intercom.wav')
	stream.pipe(fs.createWriteStream(intercomFile));
}

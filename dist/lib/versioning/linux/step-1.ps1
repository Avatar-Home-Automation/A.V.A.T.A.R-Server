...
if (Test-Path ./newVersion-$version) {
    Write-Host "⏳​ Remove old existing server master package" -NoNewline -ForegroundColor DarkMagenta
    Remove-Item ./newVersion-$version -Recurse -Force
    Write-Host " done" -ForegroundColor Green
}
Write-Host "⏳​ Extracting server master package" -NoNewline -ForegroundColor DarkMagenta
...
If (($installType -eq "exe") -or ($installType -eq "module")) {
    ...
    If ($installType -eq "exe") {
        ...
        start-process -FilePath "npx" -ArgumentList "electron-packager", ".", "--electron-version=$electron_version", "--no-asar", "--overwrite", "--icon=./avatar.ico", "--out=./output" -NoNewWindow -workingdirectory . -Wait
...
Write-Host "█                  As the server was started with this terminal,               █" -ForegroundColor DarkMagenta
Write-Host "█                   if you close it, you also close the server!                █" -ForegroundColor DarkMagenta
Write-Host "█                                                                              █" -ForegroundColor DarkMagenta
...
start-process -filePath "$app_location/A.V.A.T.A.R-Server" -workingdirectory "$app_location" -wait
Write-Host " done" -ForegroundColor Green
Start-Sleep -Seconds 1

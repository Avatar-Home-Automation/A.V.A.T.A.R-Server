...
function CheckDependencies {
    ...
    if ($PSVersionTable.PSVersion -lt [Version]"7.0") {
        ...
        Write-Host "Please, set the PowerShell executable field in the settings." -ForegroundColor Red
        Stop-Transcript
        Exit 1
    }
}
...
If (($installType -eq "exe") -or ($installType -eq "module")) {
    ...
    If ($installType -eq "exe") {
        ...
        start-process -FilePath "npx.cmd" -ArgumentList "electron-packager", ".", "--electron-version=$electron_version", "--no-asar", "--overwrite", "--icon=./avatar.ico", "--out=./output" -NoNewWindow -workingdirectory . -Wait

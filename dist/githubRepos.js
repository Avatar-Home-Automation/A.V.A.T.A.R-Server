...
function getlogin(safeStorage) {
    return new Promise(function (resolve) {
        let login =  fs.readJsonSync(path.resolve (__dirname, 'lib/github/github.json'), { throws: false })
        if (login) {
             var decrypted
...
function savelogin(safeStorage, arg) {
    return new Promise(function (resolve) {
        var encrypted
    ...
    return new Promise(async (resolve) => { 
        ...
        repo.contents('update/newVersion.txt', async (err, data) => {
            ...
            try {
                ...
                let newVersion = fs.readFileSync(newVersionFile, 'utf8');
                ...
                for (let i=0; i<3; i++) {
                    if (parseInt(currentVersion[i]) < parseInt(newVersion[i])) {
                        resolve(true);
                    }
                }
